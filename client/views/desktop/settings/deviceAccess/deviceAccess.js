angular.module('homeautomation')
    .controller('deviceAccessSettings', ['$scope', '$stateParams', '$meteor', 'ModalService',
        function($scope, $stateParams, $meteor, ModalService) {
            ...
            $scope.friendViewer = {
                events: {
                    onFriendClicked: function(friend) {
                        $scope.selectedUser = friend;
                    }
                }
            }
            ...
            $scope.deviceAccessGiven = $meteor.collection(function() {
                var controlUnits = ControlUnits.find({
                    owner: Meteor.userId()
                }).fetch();

                var controlUnitIds = _.pluck(controlUnits, 'controlUnitId');
            ...
            $scope.deviceAccessReceived = $meteor.collection(function() {
                var selectedUserId = ($scope.selectedUser) ? $scope.selectedUser._id : undefined;
